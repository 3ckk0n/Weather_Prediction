...
def getHistoricalData_API():
    ...
    while (start_date <= end_date):
        ...
        response = requests.get("http://api.weatherapi.com/v1/history.json?key=aa22924bef8642b1b20151204242711&q=Saigon&dt=" + str(start_date))
...
def getlabel(code):
    ...
    return temp
def getFuturelabel(day,lat,lon):
    response = requests.get("https://api.weatherapi.com/v1/forecast.json?key=aa22924bef8642b1b20151204242711&q=${str(lat)},${str(lon)}&days=${str(day)}&aqi=no&alerts=no")
    if response.status_code == 200:
        # Get json data from request
        data = response.json()
        # json_object = json.dumps(data, indent=4)
